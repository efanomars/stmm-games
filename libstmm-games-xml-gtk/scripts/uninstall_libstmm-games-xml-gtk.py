...
# File:   uninstall_libstmm-games-xml-gtk.py

# Removes all files installed by the install_libstmm-games-xml-gtk.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-games-xml-gtk.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f       {}/include/stmm-games-xml-gtk".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-games-xml-gtk.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-games-xml-gtk.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f {}/lib/pkgconfig/stmm-games-xml-gtk.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f  {}/share/doc/libstmm-games-xml-gtk".format(sSudo, sInstallDir).split())
	...
	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-games-xml-gtk/build".format(sSudo).split())
		subprocess.check_call("{} rm -r -f libstmm-games-xml-gtk/examples/omni/build".format(sSudo).split())
