...
def testAll(sBuildType, sInstallDir, sSudo, bBuildStatic, bSanitize):
	...
	subprocess.check_call("./scripts/uninstall_stmm-games-all.py -y --installdir {}  {}".format(sInstallDir, sSudo).split())

	subprocess.check_call("./scripts/install_stmm-games-all.py -b {}  -t On  -d On --docs-to-log  --installdir {}  {} {} {}"\
			.format(sBuildType, sInstallDir, sSudo, sStatic, sSanitize).split())


	os.chdir("libstmm-games/build")
	subprocess.check_call(sSanitizeOptions + " make test", shell=True)
	if os.path.getsize("libstmm-games_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-games_doxy.log not empty")
	os.chdir("../..")

	#os.chdir("libstmm-games-fake/build")
	#subprocess.check_call(sSanitizeOptions + " make test", shell=True)
	#if os.path.getsize("libstmm-games-fake_doxy.log") > 0:
		#raise RuntimeError("Error: libstmm-games-fake_doxy.log not empty")
	#os.chdir("../..")

	os.chdir("libstmm-games-file/build")
	subprocess.check_call(sSanitizeOptions + sSanitizeIgnoreOptions + " make test", shell=True)
	if os.path.getsize("libstmm-games-file_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-games-file_doxy.log not empty")
	os.chdir("../..")

	os.chdir("libstmm-games-gtk/build")
	subprocess.check_call(sSanitizeOptions + sSanitizeIgnoreOptions + " make test", shell=True)
	if os.path.getsize("libstmm-games-gtk_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-games-gtk_doxy.log not empty")
	os.chdir("../..")

	os.chdir("libstmm-games-xml-base/build")
	subprocess.check_call("make test", shell=True)
	if os.path.getsize("libstmm-games-xml-base_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-games-xml-base_doxy.log not empty")
	os.chdir("../..")

	os.chdir("libstmm-games-xml-game/build")
	subprocess.check_call("make test", shell=True)
	if os.path.getsize("libstmm-games-xml-game_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-games-xml-game_doxy.log not empty")
	os.chdir("../..")

	os.chdir("libstmm-games-xml-gtk/build")
	subprocess.check_call("make test", shell=True)
	if os.path.getsize("libstmm-games-xml-gtk_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-games-xml-gtk_doxy.log not empty")
	os.chdir("../..")

	if (not bSanitize):
		os.chdir("libstmm-games-xml-gtk/examples/omni")
...
def main():
	...
	print("--------------------------------")
	print("testall.py (stmm-games) started!")
	print("--------------------------------")
	...
	print("----------------------------------------------")
	print("testall.py (stmm-games) finished successfully!")
	print("----------------------------------------------")
