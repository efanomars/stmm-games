...
# File:   uninstall_libstmm-games-xml-base.py

# Removes all files installed by the install_libstmm-games-xml-base.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-games-xml-base.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f       {}/include/stmm-games-xml-base".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-games-xml-base.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-games-xml-base.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f {}/lib/pkgconfig/stmm-games-xml-base.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f  {}/share/doc/libstmm-games-xml-base".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-games-xml-base/build".format(sSudo).split())
