...
# File:   uninstall_libstmm-games-file.py

# Removes all files installed by the install_libstmm-games-file.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-games-file.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f       {}/include/stmm-games-file".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-games-file.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f        {}/lib/libstmm-games-file.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f {}/lib/pkgconfig/stmm-games-file.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f  {}/share/doc/libstmm-games-file".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-games-file/build".format(sSudo).split())
